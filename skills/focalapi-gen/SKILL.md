...
- `gpt-image-2` requires an explicit `--quality` (low/medium/high) — the provider cannot preserve the official auto default and the gateway rejects omitted/auto before billing. `--size auto` (the default) passes the official auto semantics through; explicit WIDTHxHEIGHT must satisfy the official constraints (multiples of 16, max edge 3840, ratio <= 3:1, 0.66-8.29 MP) plus the provider's 1024px minimum edge.
- `workflow-face-swap` takes exactly two images (`--image <target> <source>`: target body/scene first, the face to transplant second) and always generates one image; an optional prompt overrides the built-in head-swap instruction.
