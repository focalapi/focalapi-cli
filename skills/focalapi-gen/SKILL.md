...
- `gpt-image-2` quality defaults to auto (effective tier medium on this channel; explicit low/medium/high passes through). `--size auto` (the default) passes the official auto semantics through; explicit WIDTHxHEIGHT must satisfy the official constraints (multiples of 16, max edge 3840, ratio <= 3:1, 0.66-8.29 MP) plus the provider's 1024px minimum edge.
