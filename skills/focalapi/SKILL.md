...
| 需求 | 命令 | 子技能 |
|---|---|---|
| 生成或编辑图像 | `focalapi gen image` | focalapi-gen |
| 生成视频 | `focalapi gen video`（任务制，可 `--no-wait`） | focalapi-gen |
| 查询或下载视频任务产物 | `focalapi task status` / `focalapi task download` | focalapi-gen |
...
4. **先查模型再调用**：不能猜测模型 ID；先运行 `focalapi models list --json`。
